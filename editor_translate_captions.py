...
if not os.path.exists("output/editor_upload_result.json") or not os.path.exists("output/clip_captions_en.srt"):
...
with open("output/clip_captions_en.srt", "r") as f:
    srt_content = f.read()
...
translate_prompt = (
    ...
    "Keep numbering, timestamps, and format EXACTLY the same - only translate the subtitle text. "
...
try:
    ...
    translated_path = f"output/clip_captions_{TARGET_LANG_CODE}.srt"
    ...
    youtube.captions().insert(
        part="snippet",
        body={"snippet": {"videoId": video_id, "language": TARGET_LANG_CODE, "name": TARGET_LANG_NAME, "isDraft": False}},
